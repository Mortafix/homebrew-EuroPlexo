...
def deltabit2deltabitGen(deltabit_url):
	...
	try: get_attrs = sub(r'download_video|\'|\(|\)','',[l['onclick'] for l in soup.find_all('a') if "onclick" in l.attrs][0]).split(',')
	except IndexError: raise SiteError('DeltaBit not working!') 
...
def get_DeltaBit_download_link(url):
	try: return deltabitGen2deltabitCloud(deltabit2deltabitGen(encrypt2deltabit(url)))
